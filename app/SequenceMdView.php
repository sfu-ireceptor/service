...
class SequenceMdView extends Model
{
    ...
    public static function getSequencesQuery($f)
    {
    	...
    	$query = new SequenceMdView();
    
    	$num_results = self::$max_results;
    	$start_at = 0;
    
    	SequenceMdView::parseFilter($query, $f);
    ...
    public static function createCsvGW($filter)
    {
    	...
    	$query = new SequenceMdView();
    	SequenceMdView::parseFilter($query, $filter);
    	...
    	for ($i=1; $i <= $nbPages; $i++) {
    		...
    		$time_start_find = microtime(true);
    		$sequence_query = new SequenceMdView();
