...
use Log;
class CloneDataView extends Model {
	protected $table = "clone_data_view";
	private static $max_results = 50;
	public static $coltype = Array(
		 "clone_data_id"             => "int" ,      
		 "cd_analysis_id" => "int" ,      
		 "vgene_family"   => "string",
		 "vgene_gene"     => "string",
	...
	public static function getClonesQuery($f)
	{
		...
		$query = new CloneDataView();
		
		$num_results = self::$max_results;
		$start_at = 0;

		CloneDataView::parseFilter($query, $f);
	...
	public static function createCsvGW($filter)
	{
		...
		$query = new CloneDataView();
		CloneDataView::parseFilter($query, $filter);
		...
		for ($i=1; $i <= $nbPages; $i++) {
			...
			$time_start_find = microtime(true);
			$sequence_query = new CloneDataView();
